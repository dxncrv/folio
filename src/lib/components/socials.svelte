...
<div class="socials">
	{#each socials as social}
		{@render socialLink(social)}
	{/each}
</div>

{#snippet socialLink(social: Social)}
	<a href={social.href} target="_blank" rel="noopener noreferrer" aria-label={social.label}>
		<iconify-icon icon={social.icon} width="28" height="28"></iconify-icon>
	</a>
{/snippet}

<style>.socials{margin:0 2rem;display:flex;color:var(--accent-dim);gap:1.5rem}.socials a{font-size:1.5rem;color:var(--accent-dim);transition:color 0.2s;width:28px;height:28px;display:inline-block;text-align:center}.socials a:hover{color:var(--accent)}@media(max-width:768px){.socials{width:100%;justify-content:center;margin:1rem 0}}</style>